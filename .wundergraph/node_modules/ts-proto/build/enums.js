...
function generateEnum(ctx, fullName, enumDesc, sourceInfo) {
    ...
    chunks.push(ts_poet_1.code `export ${options.constEnums ? 'const ' : ''}enum ${fullName} {`);
...
function generateEnumToNumber(fullName, enumDesc) {
    const chunks = [];
    chunks.push(ts_poet_1.code `export function ${case_1.camelCase(fullName)}ToNumber(object: ${fullName}): number {`);
    chunks.push(ts_poet_1.code `switch (object) {`);
    for (const valueDesc of enumDesc.value) {
