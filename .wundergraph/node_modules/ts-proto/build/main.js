...
function makeTimestampMethods(options, longs) {
    ...
    `);
    const fromJsonTimestamp = ts_poet_1.conditionalOutput('fromJsonTimestamp', options.useDate
        ? ts_poet_1.code `
...
function generateDecode(ctx, fullName, messageDesc) {
  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        ...
        else if (types_1.isTimestamp(field) && options.useDate) {
            const type = types_1.basicTypeName(ctx, field, { keepValueType: true });
        ...
        const readSnippet = (from) => {
            if (types_1.isEnum(field)) {
                ...
                return options.useDate
                    ? ts_poet_1.code `${from} !== undefined ? ${from}.toISOString() : null`
                    : ts_poet_1.code `${from} !== undefined ? ${utils.fromTimestamp}(${from}).toISOString() : null`;
            }
                ...
                if (types_1.isEnum(valueType)) {
                    ...
                    return options.useDate ? ts_poet_1.code `${from}.toISOString()` : ts_poet_1.code `${utils.fromTimestamp}(${from}).toISOString()`;
                }
...
function generateFromPartial(ctx, fullName, messageDesc) {
  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        const fieldName = case_1.maybeSnakeToCamel(field.name, options);
        const readSnippet = (from) => {
            if (types_1.isPrimitive(field) || types_1.isValueType(ctx, field)) {
                return ts_poet_1.code `${from}`;
            }
            else if (types_1.isTimestamp(field) && options.useDate) {
                return ts_poet_1.code `${from}`;
                ...
                if (types_1.isRepeated(field) && types_1.isMapType(ctx, messageDesc, field)) {
                    ...
                    else if (types_1.isTimestamp(valueType) && options.useDate) {
                        return ts_poet_1.code `${from}`;
                    }
                    else {
                        const type = types_1.basicTypeName(ctx, valueType);
